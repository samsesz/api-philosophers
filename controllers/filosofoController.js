...
const createFilosofo = async (req, res) => {
  try {
    const { title, year, corrente, descriptions } = req.body;
    await filosofoService.Create(title, year, corrente, descriptions);
...
const updateFilosofo = async (req, res) => {
  try {
    if (ObjectId.isValid(req.params.id)) {
      const id = req.params.id;
      const { title, year, corrente, descriptions } = req.body;
      const game = await gameService.Update(id, title, year, corrente, descriptions);
